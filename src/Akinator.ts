...
interface question {
 question: string,
 answers: ('نعم' | 'لا' | 'لا أعرف' | 'ربما' | 'ربما لا' | string)[]
}
...
export default class Akinator {
 ...
 answers: ('نعم' | 'لا' | 'لا أعرف' | 'ربما' | 'ربما لا')[] | guess[];
 guessCount: number;
 ...
 throw new Error('يرجى تحديد منطقة صحيحة. يمكنك استيراد المناطق التي أ支持ها أو عرض الوثائق. ثم استخدمها مثل: new Aki({ region })');
 }
 ...
 if (!server) throw new Error(`تعذر العثور على خادم يتطابق مع المنطقة ${this.region}`);
